...
if (isset($_POST['action']) && $_POST['action'] === 'save_output') {
    ...
    $chain_id = isset($_POST['chain_id']) && $_POST['chain_id'] !== 'null' ? (int)$_POST['chain_id'] : null;
    ...
    try {
        if ($is_legacy_system) {
            // บันทึกลงตาราง project_outputs (ระบบเดิม) - สร้างแถวใหม่เสมอ
            $insert_query = "INSERT INTO project_outputs (project_id, output_id, output_details, created_by, created_at) 
                           VALUES (?, ?, ?, ?, NOW())";
            $insert_stmt = mysqli_prepare($conn, $insert_query);
            mysqli_stmt_bind_param($insert_stmt, 'iiss', $project_id, $output_id, $output_details, $user_id);
            $result = mysqli_stmt_execute($insert_stmt);
            mysqli_stmt_close($insert_stmt);
        } else {
            // บันทึกลงตาราง impact_chain_outputs (ระบบใหม่)
            $insert_query = "INSERT INTO impact_chain_outputs (chain_id, output_id, output_details, created_by, created_at) 
                           VALUES (?, ?, ?, ?, NOW())
                           ON DUPLICATE KEY UPDATE 
                           output_details = VALUES(output_details), updated_at = NOW()";
            $insert_stmt = mysqli_prepare($conn, $insert_query);
            mysqli_stmt_bind_param($insert_stmt, 'iiss', $chain_id, $output_id, $output_details, $user_id);
            $result = mysqli_stmt_execute($insert_stmt);
            mysqli_stmt_close($insert_stmt);
        }
